...
def make_min_input():
	...
	#############################################################################
	#                                                                           #
	#  This makes the input file for the minimization steop, if the parameters  #
...
def make_files(numFiles):
	...
	cad = make_input('1', '0', options.temp, str(options.mdSteps*options.jump), "':"+str(1+options.jump)+"-"+str(AA_NUM)+"'", '":1-'+str(AA_NUM)+'"')
	arch.write(cad)
	arch.close()
	i = 2
	while i<numFiles[0]:
		...
		arch = open(fileName, 'w')
		cad = make_input('5', '1', options.temp, str(options.mdSteps*options.jump*i), "':"+str(1+(options.jump*i))+"-"+str(AA_NUM)+"'", '":1-'+str(AA_NUM)+'"')
		arch.write(cad)
		arch.close()
		i += 1
	for cont in range(0, numFiles[1]):
		fileName = options.outDir + 'md-' + str(i).rjust(2, '0') + '.in'
		arch = open(fileName, 'w')
		cad = make_input('5', '1', options.temp, str(options.mdSteps*AA_NUM), "':"+str(AA_NUM)+"'", '":1-'+str(AA_NUM)+'"')
		arch.write(cad)
		arch.close()
		i += 1
	fileName = options.outDir + 'md-' + str(i).rjust(2, '0') + '.in'
	arch = open(fileName, 'w')
	cad = make_input('5', '1', options.temp, str(options.mdSteps*AA_NUM*2), '', '')
	arch.write(cad)
	arch.close()

def make_input(ntx, irest, temp, steps, restrM, bellyM):
	"""
	#############################################################################
	#                                                                           #
...
sander -O -i min.in -o min.out -r min.rst -c """
	...
	scri += 'sander -O -i md-01.in -o md-01.out -c min.rst -p %s.prmtop -r md-01.rst -x md-01.nc -ref min.rst\n\n' %(protFun)
	...
	while i<tope:
		...
		scri += 'sander -O -i md-%s.in -o md-%s.out -c md-%s.rst -p %s.prmtop -r md-%s.rst -x md-%s.nc -ref md-%s.rst\n\n'\
	...
	if numFiles[2] == True:
		...
		scri += 'moveX.py -f md-%s.rst -n %s -p %s\n'\
		 %(str(i-1).rjust(2, '0'), str((aaNum%options.jump)*jumpFactor), options.inputFile)
		tope = numFiles[1]
	scri += 'sander -O -i md-%s.in -o md-%s.out -c md-%s.rst -p %s.prmtop -r md-%s.rst -x md-%s.nc -ref md-%s.rst\n\n'\
	 %(str(i).rjust(2, '0'), str(i).rjust(2, '0'), str(i-1).rjust(2, '0'), protFun, str(i).rjust(2, '0'), str(i).rjust(2, '0'), str(i-1).rjust(2, '0'))
	i += 1
	...
	scri += 'sander -O -i md-%s.in -o md-%s.out -c md-%s-noFun.rst -p %s.prmtop -r md-%s.rst -x md-%s.nc -ref md-%s-noFun.rst\n\n'\
...
if __name__ == '__main__':
 ...
 &cntrl
    imin=0,
    ntx= irest= ntrx=1,
    ntxo=1, ntpr=500, ntave=0, ntwr=0,
